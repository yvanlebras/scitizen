...
exports.rescale = function(image_id, callback) {
  scitizen_storage.get(image_id, function(err, result) {
      if(err>0) {
        ...
        projects_db.findOne({ _id: image.project},
                            function(err, project){
          if(err) {
            callback(500);
            return;
          }
          var plan = CONFIG.plans[project.plan];
          ...
          gm(image_tiny_path).size(function(err, value){
              if(value.width>plan.tiny.max_width ||
                  value.height>plan.tiny.max_height) {
                  if(value.width>=value.height) {
                    gm(image_tiny_path).resize(plan.tiny.max_width);
                  }
                  else {
                    gm(image_tiny_path).resize(null,plan.tiny.max_height);
                  }
              }
